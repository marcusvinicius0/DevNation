...
import { FiCheck, FiX, SiHtml5 } from 'react-icons/fi';
				...
				<div className={styles.selectStacks}>
					{allStacks.map( (stack, index) => (
						<div className={styles.cardCheckStack} key={index}>
							<button className={stacksAlreadyAdded.indexOf(stack.stack) > -1 && styles.activeButton} onClick={() => handleAddStack(stack.stack)}><FiCheck /></button>
							<i>{stack.icon}</i>
							<span>{stack.stack}</span>
						</div>
					))}
				</div>
				<button className={styles.saveButton} onClick={handleSaveStacks}>Salvar</button>
			</div>
		</div>
	)
}